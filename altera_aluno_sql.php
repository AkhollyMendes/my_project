...
$erro = null;
		...
		$busca = $conexao->prepare('select id, email, sexo from tb_alunos where (email = :email && id != :id)');
		...
		if ($busca->rowcount() == 0)
		{
			$atualiza = $conexao->prepare("update tb_alunos set nome = :nome, sobrenome = :sobrenome, dataNascimento = :dataNascimento, sexo = :sexo, email = :email, fone = :fone, endereco = :endereco, cidade = :cidade, uf = :uf, pais = :pais, universidade = :universidade, curso = :curso, areadoestudo = :areadoestudo, anodeinicio = :anodeinicio, fileupload = :fileupload, senha = :senha where (id = :id) ");
			...
			if ($atualiza)
			{
				...
				$imgprofile = $conexao->prepare('select fileupload from tb_alunos where (id = :id)');
